...
class Target {
   ...
   * @param {?(String)} opts.executableName
  ...
  constructor(platform, pathspec, opts) {
    ...
    this.binary = path.join(
      this.output,
      'pkg',
      opts.executableName || basename.replace(extname, '')
    )
  ...
  build(callback) {
    ...
    let response = null
    steps.push((next) => this.builder.open(next))
    ...
    steps.push((next) => this.builder.close(next))
