...
function dmg (target, opts) {
  ...
  opts = Object.assign({
    ...
    executableName: null,
    icon: null,
    bundleId: 'org.littlecorelabs.pkgpackager',
    version: '1.0.0',
    copyright: null
  })

  assert(opts.output, 'output path required')
  assert(opts.productName, 'productName option required')
  assert(opts.executableName, 'executableName option required')
  assert(opts.icon, 'icon path option required')
  assert(opts.bundleId, 'bundleId option required')
  assert(opts.version, 'version option required')
  assert(opts.copyright, 'copyright statement option required')
